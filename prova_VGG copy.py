...
def build_model(input_shape_1, input_shape_2):
    ...
    x = tfkl.Dense(16, activation='relu',kernel_initializer = tfk.initializers.HeUniform(seed))(x)
    x = tfkl.Dropout(0.5)(x)

    outputs = tfkl.Dense(6,activation='linear')(x)

    tl_model = tfk.Model(inputs=[inputs_1,inputs_2], outputs=outputs, name='model')
    tl_model.compile(loss=tfk.losses.MeanSquaredError(), optimizer=tfk.optimizers.Adam(0.001), metrics=['mae'])
...
history = model.fit(
    ...
    callbacks = [tfk.callbacks.EarlyStopping(monitor='val_mae', mode='min', patience=12, restore_best_weights=True),
                 tfk.callbacks.ReduceLROnPlateau(monitor='val_loss',patience=6,factor=0.9,mode='min', min_lr=1e-6)]
).history
...
plt.plot(history['val_loss'], label='Validation', alpha=0.8, color='#4D61E2', linewidth=3) #come faccio ad inserirla nella loss che ho definito come funzione ??????
...
plt.plot(history['mae'], label='Training', alpha=0.8, color='#ff7f0e', linewidth=3)
plt.plot(history['val_mae'], label='Validation', alpha=0.8, color='#4D61E2', linewidth=3)
plt.legend(loc='upper left')
plt.title('MEA')
plt.grid(alpha=0.3)
